...
export default function Quiz({ title, description, questions, icon, gradient, onComplete }: QuizProps) {
  ...
  if (!isStarted) {
    return (
      <Card className={`bg-gradient-to-br ${gradient} border-none shadow-lg hover:shadow-xl transition-shadow duration-300`}>
        <CardHeader>
          ...
          <CardTitle className="text-2xl font-heading text-secondary">{title}</CardTitle>
        </CardHeader>
        <CardContent>
