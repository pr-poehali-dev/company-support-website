...
define('EMAIL_USER', 'info@glavbuhvl.ru'); // Замените на ваш email
define('EMAIL_PASSWORD', 'your_app_password'); // Замените на пароль приложения
define('EMAIL_TO', 'lore07061994@gmail.com'); // Замените на email получателя
...
// Отправляем email
try {
    $result = sendEmail($subject, $htmlBody, EMAIL_TO, EMAIL_USER, EMAIL_HOST, EMAIL_PORT, EMAIL_USER, EMAIL_PASSWORD);
...
function sendEmail($subject, $htmlBody, $to, $from, $host, $port, $username, $password) {
    ...
    try {
        ...
        $mail->setFrom($from, 'ГЛАВБУХВЛ Сайт');
        $mail->addAddress($to);
